...
<Info>A terminal size is a size that is either the minimum or maximum size of the box on either axis. This can happen when
the user tries to resize the box beyond the given **constraints** or the size reaches the `clampingRect` size.</Info>
